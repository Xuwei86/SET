...
class ACG(nn.Module):
    ...
    def forward(self, x):
        m = 1.5 ; n = 0.25
        b, c, h, w = x.size()
        y = ((self.avg(x))*m + (self.max(x)*n)).view([b, 1, c])
        y = self.conv(y)
...
class SETBlock(nn.Module):
    def __init__(self,
        ...
        self.attn0 = EMHSA(inchannel // 2, inchannel // 2)
        self.attn1 = channel_atten(inchannel // 2)

        self.drop_path = DropPath(drop_path_ratio) if drop_path_ratio > 0. else nn.Identity()

        self.drop_path_0 = DropPath(drop_path_ratio) if drop_path_ratio > 0. else nn.Identity()
        self.drop_path_1 = DropPath(drop_path_ratio) if drop_path_ratio > 0. else nn.Identity()
...
def SET_N_small(num_classes: int = 1000):
...
def SET_T_small(num_classes: int = 1000):
    # pretrain weight link
...
def SET_S_small(num_classes: int = 1000):
    # pretrain weight link
